defmodule DhcpServer.Handler do
      ...
      {:ok, offered_ip, opts} ->
        reply =
          ...
          |> Map.update!(:options, fn o ->
            o
            |> Map.put(:message_type, 2)  # DHCPOFFER
            |> Map.merge(opts)
          end)
        ...
        case LeaseManager.confirm_lease(mac, requested_ip, packet.xid) do
          {:ok, ip, opts} ->
            reply =
              ...
              |> Map.update!(:options, fn o ->
                o
                |> Map.put(:message_type, 5)  # DHCPACK
                |> Map.merge(opts)
              end)
  ...
  defp send_reply(reply_struct) do
    # Set broadcast flag in the reply if client has no IP yet
    reply_with_flag =
      if reply_struct.ciaddr == <<0, 0, 0, 0>> do
        # Set the broadcast bit (0x8000) in flags so client knows to accept it
        %{reply_struct | flags: 0x8000}
      else
        reply_struct
      end

    binary = Packet.build(reply_with_flag)

    # Destination logic per RFC 2131:
    # - ciaddr set (renewal/rebind): unicast to ciaddr
    # - ciaddr unset: broadcast (client has no IP yet)
    dest =
      if reply_struct.ciaddr != <<0, 0, 0, 0>> do
    ...
    Logger.debug("📤 Sending #{byte_size(binary)}B to #{format_ip(dest)}")
